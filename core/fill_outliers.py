...
from .docx_tools import set_cell_text, insert_row_after, vmerge_column
...
def fill(doc_path, xlsx_path, out_path):
    ...
    # Carry the last non-empty KPI label downward (template merges leave the
    # KPI cell blank on continuation rows). This rebuilds the per-row KPI label.
    last_kpi = ""
    for d in data:
        if d["kpi"]:
            last_kpi = d["kpi"]
    ...
    # Build Categorization groups: each distinct (kpi,cat) that has a block key.
    groups = []
    for d in data:
        if d["key"] is None:
            # keep PTH / unmapped rows so their labels are preserved, but with
            # zero patients (they stay as single labelled rows).
            groups.append({"key": None, "kpi": d["kpi"], "cat": d["cat"],
                           "start": d["idx"], "pts": []})
            continue
        groups.append({"key": d["key"], "kpi": d["kpi"], "cat": d["cat"],
                       "start": d["idx"],
                       "pts": block_data.get(d["key"], [])})

    # Process bottom-up so row insertions don't shift indices above.
    # PHASE 1: expand rows and stamp ALL text (no merging yet).
    total = 0
    for g in reversed(groups):
        ...
        for _ in range(len(pts) - 1 if len(pts) > 1 else 0):
            insert_row_after(table, start)

        cnt = str(len(pts)) if pts else ""
        for i in range(n):
            ...
            if i < len(pts):
                nid, val, bkey = pts[i]
                set_cell_text(row.cells[3], f"{nid}: {_fmt_value(val, bkey)}.")
                total += 1
            else:
