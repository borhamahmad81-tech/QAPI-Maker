...
def clone_row(table, src_row_idx, insert_after_idx=None):
    """Deep-copy the row at src_row_idx (for its formatting), and insert the
    copy directly after insert_after_idx (defaults to src_row_idx). Returns
    the new row."""
    if insert_after_idx is None:
        insert_after_idx = src_row_idx
    src_tr = table.rows[src_row_idx]._tr
    new_tr = copy.deepcopy(src_tr)
    anchor_tr = table.rows[insert_after_idx]._tr
    anchor_tr.addnext(new_tr)
    return table.rows[insert_after_idx + 1]


def insert_row_after(table, insert_after_idx, src_row_idx=None):
    """Clone formatting of src_row_idx (defaults to insert_after_idx) and
    insert the new row directly after insert_after_idx. Text is blanked."""
    new_row = clone_row(table, src_row_idx if src_row_idx is not None else insert_after_idx,
                        insert_after_idx)
