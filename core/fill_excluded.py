...
from .action_plan_templates import load_templates
...
# Human-readable explanation for each exclusion code. Applied ONLY to newly
# added rows (a freshly-excluded patient who wasn't already in the Doc) so we
# never touch or reformat any code on an already-present, reconciled row.
EXCLUSION_CODE_LEGEND = {
    "Dvexclusion_001": "Parathyroidectomy performed",
    "Dvexclusion_002": "Parathyroidectomy scheduled",
    "Dvexclusion_003": "Parathyroidectomy Planned",
    "Dvexclusion_004": "Documented non-compliance to medications or diet.",
    "Dvexclusion_005": "Documented missing > 30% of dialysis sessions during the review period",
    "Dvexclusion_006": "Documented Chronic Active bleeding",
    "Dvexclusion_007": "Documented malignancy",
    "Dvexclusion_008": "Documented blood transfusion dependence others",
    "Dvexclusion_009": "Documented Hemoglobinopathies blood transfusion dependence",
    "Dvexclusion_010": "Documented patient with HB > 12g/dl continuously without ESA",
    "VAC052": "Documented Terminal Patients.",
    "VAC053": "Documented vascular access failure.",
    "VAC054": "Documented Patient's refusal.",
    "VAC055": "Documented medically un-fit.",
}


def _resolve_action_plan(code, templates):
    """Look up saved Action Plan template(s) for a code cell that may contain
    several comma-separated codes. If more than one code has a saved template,
    join them; codes without a saved template contribute nothing. Returns ''
    if no template applies (Action Plan stays blank, as before)."""
    if not code or not templates:
        return ""
...
def _annotate_code(code):
    """Append the legend explanation to a code, e.g.
    'Dvexclusion_010' -> 'Dvexclusion_010 (Documented patient with HB > 12g/dl
    continuously without ESA)'. A cell can hold several comma-separated codes;
    each recognised one gets annotated. Unknown codes (e.g. vascular free text
    like 'Unfit') pass through unchanged."""
    if not code:
        return code
    parts = [p.strip() for p in code.split(",") if p.strip()]
    out = []
    for p in parts:
        explanation = EXCLUSION_CODE_LEGEND.get(p)
        out.append(f"{p} ({explanation})" if explanation else p)
    return ", ".join(out)
...
def fill_table(doc, xlsx_path):
    ...
    for cat_key, keys in by_cat.items():
        ...
        for key in keys:
            ...
            set_cell_text(new_row.cells[2], _annotate_code(e["code"]))
            set_cell_text(new_row.cells[3], _resolve_action_plan(e["code"], templates))
            added += 1
